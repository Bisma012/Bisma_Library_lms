...
def load_library():
    try:
        if os.path.exists('library.json'):
            if os.path.getsize('library.json') == 0:  # Check if the file is empty
                st.session_state.library = []
                save_library()  # Initialize with an empty list
            else:
                with open('library.json', 'r') as file:
                    st.session_state.library = json.load(file)
            return True
        return False
    except json.JSONDecodeError:  # Handle corrupted JSON files
        st.error("Error loading library: The file is corrupted. Reinitializing...")
        st.session_state.library = []
        save_library()  # Reinitialize the file
        return False
...
st.markdown("---")
st.markdown("Copyright @ 2025 Bisma Waheed Personal Library Manager Created with love and Streamlit💗", unsafe_allow_html=True)
